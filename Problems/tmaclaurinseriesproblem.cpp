...
#ifdef __DEBUG__
    std::cout << asNumeric("x").CodeBasic() << endl;
#endif
    for (size_t i = 0; i < Coefs.size(); i++) {
        TPolynomSeries Term(Power * Coefs[i]);
#ifdef __DEBUG__
        std::cout << Term.asNumeric("x").CodeBasic() << endl;
#endif
        Res = Res + Term;
        Power = Power * P;
#ifdef __DEBUG__
        std::cout << Power.asNumeric("x").CodeBasic() << endl;
        std::cout << Res.asNumeric("x").CodeBasic() << endl;
#endif
    }
...
TNumeric TPolynomSeries::asNumeric(const string& var) {
    TPolynom This(Coefs);
...
void TMaclaurinSeriesProblem::BuildPhrases() {
    ...
    MyTranslator.AddEng("Can not find Maclaurin polynomial for %N, because base of power function should be non-zero when %n in "
                        "vicinity of zero");
            ...
            if (Arg0 == 0) {
                ...
                if (ExpandIntoSeries(Writer, &Arg, var, *N.operands[0], LastTerm)) {
                    ...
                    if (Writer) {
                        ...
                        Writer->AddFormula(MakeEquality(TNumeric("t"), t.asNumeric(var)));
                    }
                    switch (N.operation) {
                        case OperatorSin:
            ...
            if (ExpandIntoSeries(Writer, &f, var, *N.operands[0], LastTerm)) {
                ...
                if (f0 == TNumeric(0)) {
                    ...
                    *Res = Ln.Substitute(t, LastTerm) + MakeLn(f0);
                    if (Writer) {
                        Writer->AddFormula(MakeEquality(TNumeric("t"), t.asNumeric(var)));
                        Writer->AddFormula(MakeEquality(MakeLn(TNumeric("t") + TNumeric(1)), Ln.asNumeric("t")));
                        Writer->AddFormula(MakeEquality(N, Res->asNumeric(var)));
                    }
                }
            } else
                return false;
            ...
            if (Pow.DependsOn(var.c_str())) {
                ...
                if (Integer == false || P < 0) {
                    ...
                    if (Arg0 == TNumeric(0)) {
                        if (Writer) {
                            Writer->BeginError();
                            Writer->AddParagraph("Can not find Maclaurin polynomial for %N, because base of power function "
                                                 "should be non-zero when %n in vicinity of zero",
                                                 N, TNumeric(var));
                            Writer->EndError();
                        }
                        return false;
                        ...
                        if (Integer == false && Arg0.Calculate() < 0) {
                            if (Writer) {
                                Writer->BeginError();
                                Writer->AddParagraph("Can not find Maclaurin polynomial for %N, because base of power function "
                                                     "should be positive if power is fractional.",
                                                     N);
                                Writer->EndError();
                            }
                            return false;
                        ...
                        if (Arg0 != TNumeric(1)) {
                            if (Writer) {
                                ...
                                Writer->AddFormula(MakeEquality(t, tser.asNumeric(var)));
                            }
                        };
                        Res->Coefs.assign(LastTerm + 1, TNumeric("0"));
