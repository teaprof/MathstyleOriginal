...
class THomogeneousTrigEquality :
    public TPolynomConditions,
    public TEquality
// Уравнение вида
//  p_n cos^n + p_{n-1} cos^(n-1)sin + p_1 cos^(n-2)sin^2 + ...+p_0 sin^n = 0
{
  public:
    ...
    virtual string GetShortTask();
    virtual string GetClassName() {
        return "THomogeneousTrigEquality";
    };

    // virtual void SetLeftPartP(const TPolynom &P, bool AllCoef = true); //Если AllCoef = true, то будут учитываться все
    // коэфициенты, если false - то нулевые коэфициенты старше MajorPower будут отброшены virtual void SetMaxPower(size_t
    // MaxPower, int operation); //устанавливает максимальную степень многочлена MaxPower
