...
void TLinearTrigEquality::SetCoef(const TNumeric& a, const TNumeric& b, const TNumeric& c) {
    PNumeric A = Conditions->GetByRole(IDa);
    *A = a;
    A->ClearID();
    A->role = IDa;
    A->EditableFlags = ConstAllowed;
    PNumeric B = Conditions->GetByRole(IDb);
    *B = b;
    B->ClearID();
    B->role = IDb;
    B->EditableFlags = ConstAllowed;
    PNumeric C = Conditions->GetByRole(IDc);
    *C = c;
    C->ClearID();
    C->role = IDc;
    C->EditableFlags = ConstAllowed;
}
...
TNumeric THomogeneousTrigEquality::GetVarPower(size_t power) {
    ...
    if (sinpower == 0)
        return MakeCos(UnknownVar) ^ TNumeric(power);
    else {
        if (power == 0)
            return MakeSin(UnknownVar) ^ TNumeric(sinpower);
        else
            return ((MakeSin(UnknownVar)) ^ TNumeric(sinpower)) * ((MakeCos(UnknownVar)) ^ TNumeric(power));
    };
}
...
bool THomogeneousTrigEquality::GetSolution(std::shared_ptr<THTMLWriter> Writer) {
    ...
    if (P.IsZero()) {
        ...
        if (Writer)
            Writer->AddParagraph("Degenerated case: 0 = 0, consequently %N", MakeBelongsTo(UnknownVar, Result.asNumeric()));
        return true;
    } else {
