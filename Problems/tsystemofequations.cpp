...
size_t StartID = 0;
    Equation.EliminateFunctions(StartID, Map);
    Var.EliminateFunctions(StartID, Map);
    math_clear_all();
...
TNumeric LeftPart;
    ...
    for(size_t i = 0; i < Variables.size(); i++)
        FreeTerm = FreeTerm.Substitute(Variables[i].K, TNumeric("0"));
    ...
    for(size_t i = 0; i < Variables.size(); i++)
    {
        ...
        {
            if(j != i) Eq1 = Eq1.Substitute(Variables[j].K, TNumeric("0"));
        };
        Eq1 = Eq1-FreeTerm;
        Eq1 = Eq1.Simplify();
        Eq1.ID = MaxID++;
    ...
    Coefs[Variables.size()].ID = FreeTerm.ID;
    IDs[Variables.size()] = FreeTerm.ID;
    ID.push_back(IDs);
