...
vector<TNumeric> NewCoefs;
    ...
    TPolynom P(NewCoefs);
    if(Writer)Writer->AddFormula(P.GetNumeric(TNumeric("x")));
    for(size_t power = 0; power <= P.MajorPower(); power++)
        P.Coef[power] = P.Coef[power].Simplify();
    if(Writer)
        Writer->AddParagraph("After simplifying %N", P.GetNumeric(TNumeric("x")));
    Result = P;
...
TRationalIntegralProblem::TRationalIntegralProblem(const TRationalIntegralProblem& S) : TRationalFunctionConditions(*(TRationalFunctionConditions*)&S)
{
...
TPolynom Denominator = this->GetDenominatorP();
    ...
    if(Writer)Writer->AddFormula(MakeEquality(MakeFrac(GetNominatorP().GetNumeric(UnknownVar), GetDenominatorP().GetNumeric(UnknownVar)),
                             MainPart.GetNumeric(UnknownVar) + MakeFrac(Nominator.GetNumeric(UnknownVar), Denominator.GetNumeric(UnknownVar))));
//*********************РАЗЛАГАЕМ ЗНАМЕНАТЕЛЬ НА ЭЛЕМЕНТАРНЫЕ ДРОБИ И СОСТАВЛЯЕМ СУММУ ПРАВИЛЬНЫХ ДРОБЕЙ С НЕИЗВЕСТНЫМИ КОЭФФИЦИЕНТАМИ
...
vector<size_t> RootNum; //какой по номеру корень стоит в знаменателе элементарной дроби (Q.SquareMods[i] имеет номер Q.Roots.size() + i)
vector<size_t> Multiplicity; //в какой степени стоит корень
    ...
    for(size_t i = 0; i < Q.Roots.size(); i++)
        for(size_t j = 1; j <= Q.RootsMultiplicity[i]; j++)
        {
            ...
            if(j > 1) Denominator = DenominatorPowerOne^j;
            else Denominator = DenominatorPowerOne;
            ElementarFractions.push_back(TRationalFunction(Nominator, Denominator));

            TNumeric DenominatorPowerOneNumeric;
            DenominatorPowerOneNumeric = DenominatorPowerOne.GetNumeric(UnknownVar);
            TNumeric DenominatorNum = DenominatorPowerOneNumeric^j;
            ElementarFractionsNum.push_back(TPolynom(Nominator).GetNumeric(UnknownVar)/DenominatorNum);
    ...
    for(size_t i = 0; i < Q.SquareMods.size(); i++)
        for(size_t j = 1; j <= Q.SquareModsMultiplicity[i]; j++)
        {
            ...
            TNumeric DenominatorPowerOneNum = Q.SquareMods[i].GetNumeric(UnknownVar);
...
TNumeric BigSum;
    ...
    if(Writer)
        Writer->AddParagraph("Findings coefficients %n from %N", TNumeric("a_i"), MakeEquality(Nominator.GetNumeric(UnknownVar)/Denominator.GetNumeric(UnknownVar), BigSum));
...
TPolynom BigNominator;
    ...
    for(size_t i = 0; i < ElementarFractions.size(); i++)
    {
        ...
        for(size_t j = 0; j < Q.SquareMods.size(); j++)
        {
            ...
            TPolynom Denominator = DenominatorPowerOne^power;
            TNumeric DenominatorNum = Q.SquareMods[j].GetNumeric(UnknownVar);
            if(power > 1) DenominatorNum = DenominatorNum^TNumeric(power);
            if(power > 0)
            {
                MultiplicatorNum.OperandsPushback(DenominatorNum);
    ...
    if(Writer)
        Writer->AddParagraph("After casting to common denominator %N", MakeEquality(Nominator.GetNumeric(UnknownVar)/Denominator.GetNumeric(UnknownVar), BigNominatorNum));
...
//******************** СОСТАВЛЯЕМ И РЕШАЕМ СИСТЕМУ УРАВНЕНИЙ ***************************************************
   ...
   if(Writer)
       Writer->AddParagraph("Common nominator is %N", MakeEquality(Eq.GetNumeric(UnknownVar), TNumeric("0")));
...
//*************************** ВЫПИСЫВАЕМ РАЗЛОЖЕНИЕ НА ЭЛЕМЕНТАРНЫЕ ДРОБИ
    ...
    if(Writer)
        Writer->AddParagraph("The source function can be represented as %N", MakeEquality(
                        MakeFrac(GetNominatorP().GetNumeric(UnknownVar), GetDenominatorP().GetNumeric(UnknownVar)),
                        BigNominatorNum)
                    );
...
TNumeric Result;
    ...
    {        
        if(ElementarFractionsNum[i] != TNumeric("0"))
        {
            ...
            EFI.SetConditions(ElementarFractionsNum[i], UnknownVar.K);
...
TPolynomIntegralProblem PIP;
    ...
    //if(Writer)Writer->DecrementNestingLevel();
    Result.OperandsPushback(PIP.Result.GetNumeric(UnknownVar));
    ...
    if(MaskAlg.Match(Mask, Num))
    {
        ...
        if(A2 == 0)
        {
           ...
           if(N == 1)
           {
               ...
               Res = Res.Simplify();
               if(Writer)Writer->AddParagraph("After simplifying: %N", MakeEquality(MakeIntegral(Num, "x"), Res + TNumeric("C")));
               return true;
           };
        };
   };
