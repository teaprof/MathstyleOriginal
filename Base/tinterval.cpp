...
bool TInterval::TestPoint(const TNumeric& P) const
{
    ...
    if(p >= l && p <=r)
    {
        if(p > l && p < r) return true; else
        if(p == l && p != r) return IncludeLeft; else
        if(p != l && p == r) return IncludeRight; else
        //то есть if(p == l && p == r)
...
TIntervalsSet Res(*this);
    Res.Intervals.insert(Res.Intervals.end(), Set.Intervals.begin(), Set.Intervals.end()); //Res содержит все интевалы из *this и из Set
...
{
TIntervalsSet Res;
    if(I.Intervals.size() == 0) Res = *this;
    else
    {
...
TNumeric TIntervalsSet::GetNumeric()
{
TNumeric Res;
