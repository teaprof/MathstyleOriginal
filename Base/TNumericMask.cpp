...
bool TNumericMask::CheckTemplateConstant(const TNumeric& Template,
    ...
    if (Template.strval == "x") {
        ...
        {
            if (NConst.DependsOn("x"))
                return false;
            else {
                // cout<<strval<<" = "<<N.CodeBasic()<<endl;
                // проверяем, чтобы не было противоречий с уже записанными константами
                map<string, TNumeric>::iterator j = Coefs.find(Template.strval);
                if (j == Coefs.end())
                    Coefs.insert(pair<string, TNumeric>(Template.strval, NConst));
                else if (j->second != NConst)
                    return false;
                return true;
            };
        } else {
            // Число
            if (NConst.operation == OperatorConst && NConst.strval == Template.strval)
                return true;
            else
                return false;
        };
...
// operation должен быть коммутативным
{
    ...
    switch (Template.operation) {
        case OperatorSum:
            AdditionalTerm = TNumeric("0");
            break;
        case OperatorProd:
            AdditionalTerm = TNumeric("1");
            break;
        default:
            return false;
    }
...
// преобразует к произведению
{
    ...
    } else if (N.operation == OperatorProd) {
...
// преобразует к сумме
{
    ...
    } else if (N.operation == OperatorSum) {
        for (size_t i = 0; i < N.operands.size(); i++)
                ...
                if (Power->isInteger(&d) && d >= 0) {
                    switch (d) {
                        ...
                        default:
                            ...
                            //если раскрывать степень, то будет очень много слагаемых в случаях, например, (x^2+x+1)^2 --
                            CheckTemplateCommutative будет очень медленной Res.operation = OperatorProd; for(size_t i = 0; i < d;
                            i++) Res.OperandsPushback(*N.Operands[0]); Res = Expand(Res);*/
                    };
                } else
                    Res = N;
            } else
...
TNumeric TNumericMask::ToPow(const TNumeric& NConst) {
    ...
    // 1/f(x) => f(x)^(-1)
    {
        std::shared_ptr<const TNumeric> A = N.operands[0];
        std::shared_ptr<const TNumeric> B = N.operands[1];
        if (*A == TNumeric("1")) {
            N = (*B) ^ TNumeric("-1");
        } else {
            N = (*B / (*A)) ^ TNumeric("-1");
        ...
        if (N.operation == OperatorProd) {
            ...
            for (size_t i = 0; i < N.operands.size(); i++) {
                ...
                if (i == 0)
                    Power = *N.operands[i]->operands[1];
                else if (Power != *N.operands[i]->operands[1]) {
                    AllPower = false;
                    break;
                }
            }
        ...
        if (N == TNumeric(1)) {
            ...
            if (NConst.operation == OperatorProd) {
                ...
                if (AllEquals) {
                    ...
                    TempN.OperandsPushback(std::move(TNumeric(static_cast<int>(NConst.operands.size()))));
...
bool TNumericMask::Match(const TNumeric& Template, const TNumeric& N, std::map<std::string, TNumeric>& Coefs) {
    ...
    switch (Template.operation) {
        case OperatorConst:
            res = CheckTemplateConstant(Template, N, Coefs);
            break;
        case OperatorPow:
            res = CheckTemplatePow(Template, N, Coefs);
            break;
        case OperatorProd:
            res = CheckTemplateProd(Template, N, Coefs);
            break;
        case OperatorFrac:
            res = CheckTemplateFrac(Template, N, Coefs);
            break;
        case OperatorSum:
            res = CheckTemplateSum(Template, N, Coefs);
            break;
        case OperatorMinus:
