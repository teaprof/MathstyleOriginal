...
void TMyTranslator::AddEng(string str) {
    Eng.push_back(str);
    if (Rus.size() + 1 != Eng.size()) throw "TMyTranslator::AddRus(string str): AddRus must be called once after every AddEng";
}

void TMyTranslator::AddRus(string str) {
    Rus.push_back(str);
    if (Rus.size() != Eng.size()) throw "TMyTranslator::AddRus(string str): AddRus must be called once after every AddEng";
}
void TMyTranslator::AddDictionary(std::string str) {
    if (CheckDictionary(str) == false) Dictionaries.push_back(str);
}
