...
//Необходимо, чтобы A и B были положительны
{
    ...
    if((A == 1) & (B == 1))
    {
        ...
        if(A == 1) Power = k2; else
        if(B == 1) Power = k1; else
            Power = k1<k2?k1:k2;
...
int NOD = 0;
    ...
    for(size_t i = 1; i < V.size(); i++)
        if(abs(V[i])<NOD && abs(V[i]) > 0) NOD = abs(V[i]);
    while(NOD > 1)
    {
        bool Found = true;
        for(size_t i = 0; i < V.size(); i++)
...
int Divisor = 2;
    if(V.size() == 0)return 1;
    for(size_t i = 0; i < V.size(); i++) V[i] = abs(V[i]);
    do
    {
        ...
        if(IsDivisor) NOK *= Divisor; //возможно, число Divisor еще раз является делителем
        else Divisor++;  //число Divisor делителем больше не является
        if(AllOnes)break;
    }while(true);
    return  NOK;
}
