...
void QFormulaArea::SetUndoRedoEnabled() {
    if (curSaved_idx == 0)
        emit OnEnableUndo(false);
    else
        emit OnEnableUndo(true);
    if (curSaved_idx == Saved.size() - 1)
        emit OnEnableRedo(false);
    else
        emit OnEnableRedo(true);
}
...
void QFormulaArea::Simplify() {
    ...
    {
            int EditableFlags = Formula->Selected->EditableFlags;
            TNumeric *S = Formula->Selected; //это сделано на случай, если Formula будет совпадать с Selected. Тогда
    Formula->Selected=N будет вести себя неадекватно int OldID = S->role; TNumeric Res = S->Simplify();
...
QFormulaEditor::QFormulaEditor(QWidget* parent) : QWidget(parent) {
...
/*void QFormulaEditor::mouseDoubleClickEvent(QMouseEvent *)
{
    if(Formula && Formula->Selected && Formula->Selected->operation == OperatorConst)
    {
        ...
        double oldval;
        sscanf(Formula->Selected->strval.c_str(), "%lf", &oldval);
        newval = QInputDialog::getDouble(this, QString::fromUtf8("Новое значение"), QString::fromUtf8("Введите число"), oldval,
-1e3, 1e3, 4, &ok); if(ok)
        {
            char Data[100];
            sprintf(Data, "%g", newval);
